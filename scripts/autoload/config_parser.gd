...
class Type:
	...
	static func float_num(
		validation_function: Callable,
		range_min: float,
		range_max: float,
		max_precision: int,
		argument_enum: ArgEnum
	) -> Type:
		var type = new()
		...
		for line in lines:
			...
			if line.begins_with("--@description "):
				...
				(
					regex
					. compile(
						r"--@type (?<type>int|float)( precision=(?<precision>\d+))?( range=(?<range_min>[+-]?\d+)\.\.(?<range_max>[+-]?\d+))?( precision=(?<precision2>\d+))?"
					)
				)
				...
				if re_match:
					...
					if has_precision_or_range:
						...
						if re_match.get_string("type") == "int":
							...
							current_field.type = Type.integer(
								validate_with_range_int, int(range_min), int(range_max)
							)
						else:
							if precision.is_empty():
								current_field.type = Type.float_num(
									validate_with_range_float,
									float(range_min),
									float(range_max),
									0,
									ArgEnum.FLOAT_RANGE
								...
								current_field.type = Type.float_num(
									validate_with_range_precision_float,
									float(range_min),
									float(range_max),
									int(precision),
				...
				if re_match:
					...
					match type:
						...
						"int":
							current_field.type = Type.with(
								"int", func(string: String): return string.is_valid_int()
							)
						"float":
							current_field.type = Type.with(
								"float", func(string: String): return string.is_valid_float()
							)
						"bool":
							current_field.type = Type.with(
								"bool",
								func(string: String): return string == "true" or string == "false"
							)
						"string":
					...
					if (
						...
						and current_field.type
						and !current_field.type.validate(current_field.default)
					):
						warn(
							(
								"Invalid default value: %s [Line %s]"
								% [current_field.default, line_number]
							)
						)
				...
				if current_field.type and !current_field.type.validate(current_field.default):
					warn(
						"Invalid default value: %s [Line %s]" % [current_field.default, line_number]
					)
				...
				if re_match:
					...
					if !current_field.description:
						warn(
							(
								"Missing @description declaration for %s [Line %s]"
								% [current_field.name, line_number]
							)
						)
					if !current_field.type:
						warn(
							(
								"Missing @type declaration for %s [Line %s]"
								% [current_field.name, line_number]
							)
						)
					if !current_field.default:
						warn(
							(
								"Missing @default declaration for %s [Line %s]"
								% [current_field.name, line_number]
							)
						)
...
func validate_list(string: String, type: String) -> bool:
	...
	var regex = RegEx.create_from_string(
		(
			...
			% [allowed_values_regex, allowed_values_regex]
		)
	)
...
func validate_with_range_precision_float(
	string: String, range_min: float, range_max: float, max_precision: int
) -> bool:
	var regex = RegEx.create_from_string(r"^\d+(\.\d{1,%s})?$" % max_precision)
	var float_num = float(string)
